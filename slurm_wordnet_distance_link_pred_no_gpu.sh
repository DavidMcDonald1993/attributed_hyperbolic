...
#SBATCH --job-name=noGPUWordnetDistanceLinkPred
#SBATCH --output=noGPUWordnetDistanceLinkPred_%A_%a.out
#SBATCH --error=noGPUWorndetDistanceLinkPred_%A_%a.err
#SBATCH --array=0-29
...
python embedding/hyperbolic_embedding.py --dataset wordnet --data-directory ${PROJECT_DIR}/data --no-load ${ARR[${SLURM_ARRAY_TASK_ID}]} 