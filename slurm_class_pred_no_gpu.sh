...
#SBATCH --job-name=noGPUClassPred
#SBATCH --output=noGPUClassPred_%A_%a.out
#SBATCH --error=noGPUClassPred_%A_%a.err
#SBATCH --array=0-119
...
ARR=(--dataset=cora_ml" "--seed={0..29}" "--softmax" "--dim={5,10}" --evaluate-class-prediction "--{no-attributes,jump-prob=.1})
