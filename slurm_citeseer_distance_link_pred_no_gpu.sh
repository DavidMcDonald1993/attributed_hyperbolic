...
ARR=(--seed={0..29}" "--softmax" "--dim=3" --evaluate-link-prediction "--{no-attributes,jump-prob=0.05})
...
echo "starting dataset=citeseer "${ARR[${SLURM_ARRAY_TASK_ID}]}
...
echo "completed dataset=citeseer "${ARR[${SLURM_ARRAY_TASK_ID}]}