...
#SBATCH --job-name=noGPUCoraMLDirectedDistanceLinkPred
#SBATCH --output=noGPUCoraMLDirectedDistanceLinkPred_%A_%a.out
#SBATCH --error=noGPUCoraMLDirectedDistanceLinkPred_%A_%a.err
#SBATCH --array=0-29
...
python embedding/hyperbolic_embedding.py --dataset cora_ml --data-directory ${PROJECT_DIR}/data \
--no-load ${ARR[${SLURM_ARRAY_TASK_ID}]} --directed